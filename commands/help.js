...
exports.run = async (client, message, args1) => {
    ...
    .addField('🖼 - Images' + online, "`baeutiful` `blood` `bloodhelp` `blur` `blurple` `bob` `brazzers` `convinvert` `convmatrix` `convolute` `discord` `discordlogo` `displace` `distortion` `fire` `french` `gay` `ghost` `glitch` `grayscale` `green` `illuminati` `implode` `invert` `jackolantern` `magik` `orangly` `pixelate` `posterize` `presidentialalert` `rainbow` `red` `reddit` `sepia` `spotify` `steam` `time` `tobecontinued` `triggered` `triggeredinvert` `twitch` `twitter` `wasted` `youporn` ")
    .addField('🍆 - NSFW' + online, "`4k` `amateur` `hentai` `penis` `porngif` `public` `pussy`")
    .addField('💰 - Don/Economie' + online, "`coins` `pay` `slot` `daily` `work`")
    .addField('🔒 - Sécurité' + online, "`automod` `textlog`")
    ...
    .addField('🔒 - Security' + online, "`automod` `textlog`")
    .addField('🎉 - GiveAway' + online, "`giveaway`")
    .addField('🆔 - Support' + online, "`support` `report`")
