...
module.exports.run = async (client, message, args1) => {
  ...
  let ava = message.content.split(" ").slice(1)
  ava = ava.join(" ")
if(!ava) return message.channel.send('Tu dois insérer un texte')
  const arcadiaapi = require('arcadia-module');
 
  arcadiaapi.texts("presidentialalert", ava).then(url => {
  if(message.mentions.users.size > 0) return message.channel.send('Pas de mention')
...
exports.conf = {
    ...
    exports.help = {
     name: 'presidentialalert', description: 'Manipule une image.', usage: `presidentialalert [user]`
     };